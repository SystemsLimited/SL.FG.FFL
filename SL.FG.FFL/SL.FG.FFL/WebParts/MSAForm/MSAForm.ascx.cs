...
namespace SL.FG.FFL.WebParts.MSAForm
{
    ...
    public partial class MSAForm : WebPart
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!Page.IsPostBack)
                {
                    ...
                    if (!String.IsNullOrEmpty(Page.Request.QueryString["MSAID"]) || !String.IsNullOrEmpty(this.hdnMSAId.Value))
                    {
        ...
        private int GetSubmittedMSAId(SPWeb oSPWeb, int scheduleId)
        {
            try
            {
                string listName = "MSA";
                ...
                {
                    return spListItems[0].ID;
                }
            }
            catch (Exception ex)
            {
                SPDiagnosticsService.Local.WriteTrace(0, new SPDiagnosticsCategory("SL.FG.FFL(MSAForm->GetSubmittedMSAId)", TraceSeverity.Unexpected, EventSeverity.Error), TraceSeverity.Unexpected, ex.Message, ex.StackTrace);
        ...
        private SPUser UpdateMSAScheduleControls(SPWeb oSPWeb)
        {
            ...
            try
            {
                ...
                if (spListMS != null && !String.IsNullOrEmpty(this.hdnScheduleId.Value))
                {
                    ...
                    if (spListItemMS["FFLScheduleDesignation"] != null)
                    {
                        string designation = Convert.ToString(spListItemMS["FFLScheduleDesignation"]);
                    ...
                    if (spListItemMS["FFLArea"] != null)
                    {
                        string areaAudited = Convert.ToString(spListItemMS["FFLArea"]);
                        ...
                        if (!String.IsNullOrEmpty(areaAudited))
                        {
                            ...
                            {
                                this.areaAudited_ddl.SelectedValue = areaAudited;
                        ...
                        if (splistMSA != null)
                        {
                            ...
                            if (spListItemMSA != null)
                            {
                                ...
                                if (spListItemMSA["IsSavedAsDraft"] != null)
                                {
                                    ...
                                    {
                                        this.message_div.InnerHtml = "Update operation is not allowed on saved records!!! Please contact the administrator at email address - FFL.HSE@fatima-group.com";
                                        DisableControls();
                                ...
                                if (isSavedAsDraft == false)
                                {
                                    ...
                                    {
                                        string redirectUrl = "/_layouts/15/ReportServer/RSViewerPage.aspx?rv:RelativeReportUrl=/sites/ffl/Documents/MSADetailReport.rdl&rp:MSAOID=" + this.hdnMSAId.Value.Trim() + "&rv:ParamMode=Hidden";
                                            ...
                                            if (spListMS != null)
                                            {
                                                ...
                                                if (spListItemMS != null)
                                                {
                                                    ...
                                                    if (spListItemMS["End Time"] != null && spListItem["MSADate"] != null)
                                                    {
                                                        ...
                                                        {
                                                            spListItemMS["MSA Status"] = spListMS.Fields["MSA Status"].GetFieldValue("Completed");
                                                        }
                                                        else
                                                        {
                                                            spListItemMS["MSA Status"] = spListMS.Fields["MSA Status"].GetFieldValue("Completed (Late)");
                                                        }
                                                    }
